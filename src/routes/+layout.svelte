				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • Ask Docis`, {
								body: content,
								icon: `${WEBUI_BASE_URL}/static/brand-favicon.png`
							});
						}
					}
...
<svelte:head>
	...
	<link crossorigin="anonymous" rel="icon" href="{WEBUI_BASE_URL}/static/brand-favicon.png?v=3" />
